...
```{r fig8, fig.width = 10, fig.height = fig_haut_reg_poissons, fig.cap = "Evolution interannuelle des densités en poissons dans le département (en nombre d'individus par m²)."}
g <- reg_dept_capt %>% 
...
La figure \@ref(fig:fig9) indique le nombre de captures par espèce dans le département pour l’année `r  params$mon_annee`. La figure \@ref(fig:fig10) s'inscrit dans une perspective de fonctionnement trophique, et montre la biomasse cumulée de ces captures par espèce. 
...
```{r fig9, fig.height = 9, fig.cap = paste0("Effectif capturé par espèce dans le département en ", params$mon_annee,".")}
reg_dept_capt %>%
  filter(dept == params$mon_dept,
         annee == params$mon_annee,
         effectif > 0) %>% 
  ggplot(aes(x = fct_reorder(esp_code_alternatif2, effectif),
             y = effectif)) +
...
```{r fig10, fig.height = 9, fig.cap = paste0("Biomasse capturé par espèce dans le département en ", params$mon_annee,".")}
reg_dept_capt %>%
  filter(dept == params$mon_dept,
         annee == params$mon_annee,
         poids > 0) %>% 
  mutate(poids = poids / 1000) %>% 
  ggplot(aes(x = fct_reorder(esp_code_alternatif2, poids),
             y = poids)) +
  geom_bar(stat = "identity",
           fill = "darkgreen") +
  coord_flip() +
...
```{r fig11, fig.height=2.5, fig.width=5, fig.cap = "Nombre d'espèces capturées chaque année dans le département, selon leur statut."}
reg_rs %>% 
  filter(dept == params$mon_dept) %>% 
  ggplot(aes(x = annee,
             y = rs,
